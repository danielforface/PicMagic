...
const App: React.FC = () => {
  ...
  const handleGenerateFromScratch = useCallback(async (prompt: string) => {
    setIsLoading(true);
    setError(null);
    try {
  ...
  const renderContent = () => {
    ...
    if (!currentImageUrl) {
      return <StartScreen onFileSelect={handleFileSelect} onGenerateFromPrompt={handleGenerateFromScratch} isLoading={isLoading} />;
    }
